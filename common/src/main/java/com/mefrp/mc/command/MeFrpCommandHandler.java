...
public class MeFrpCommandHandler {
    ...
    public void handleCommand(String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "stop" -> runApi(this::stopTunnel);
            case "node" -> runApi(() -> beginNodeSwitch(null));
            case "status" -> runApi(this::showStatus);
            case "userinfo" -> runApi(this::showCurrentUserInfo);
            case "sign" -> runApi(this::handleSign);
            case "online" -> handleOnline(args);
            case "select" -> handleSelect(args);
            default -> platform.sendMessage("[MeFrp] 未知命令。输入 /mefrp help 查看可用命令。");
        }
    }
    ...
    private void handleStart(String[] args) {
        ...
        runApi(() -> startTunnel(finalExplicitPort));
    }
    ...
    private void handleSelect(String[] args) {
        ...
        try {
            ...
            runApi(() -> createWithSelectedNode(selection, node));
        } catch (NumberFormatException e) {
            platform.sendMessage("[MeFrp] 节点编号无效");
        }
    }
    ...
    private String regionName(String region) {
        ...
        return region == null || region.isBlank() ? "未知区域" : region;
    }
    ...
    private record PendingNodeSelection(List<Node> nodes, Map<Integer, NodeStatus> nodeStatuses, int port, Proxy oldProxy) {
    }
}
