...
public class MeFrpFabricPlatform implements MeFrpPlatform {
    private static final Logger LOGGER = LogManager.getLogger("MeFrp-MC-Mod");
    private static final ThreadLocal<CommandSourceStack> CURRENT_SOURCE = new ThreadLocal<CommandSourceStack>();
    ...
    public boolean isDedicatedServer() {
        return FabricLoader.getInstance().getEnvironmentType().name().equals("SERVER");
    }
    ...
    public void runOnMainThread(final Runnable runnable) {
        final CommandSourceStack source = CURRENT_SOURCE.get();
    ...
    public void sendClickableCommand(final String text, final String command, final String hoverText) {
        MutableComponent component = new TextComponent(text).withStyle(style -> style
    ...
    public void sendClickableCommand(String label, String text, String command, String hoverText) {
        MutableComponent component = formatMessage(label)
                .append(new TextComponent(text).withStyle(style -> style
    ...
    public void sendCopyableText(String label, String value, String hoverText) {
        MutableComponent component = formatMessage(label)
                .append(new TextComponent(value).withStyle(style -> style
    ...
    private MutableComponent formatMessage(String message) {
        if (message.startsWith("[MeFrp]")) {
            return new TextComponent("[MeFrp]").withStyle(ChatFormatting.AQUA)
                    .append(new TextComponent(message.substring("[MeFrp]".length())).withStyle(messageColor(message)));
        }
