...
public class TunnelManager {
    ...
    public List<Node> listAvailableNodes() throws ApiException {
        ...
        for (Node node : api.listNodes()) {
            if (node.isOnline && !node.isDisabled && node.supportsTcp()) {
                nodes.add(node);
            }
        }
    ...
    public String connectionAddress(Proxy proxy, ProxyList list) {
        ...
        if (hostname == null || hostname.trim().isEmpty() || hostname.contains("权限") || hostname.contains("失败") || hostname.contains("错误")) {
    ...
    private int chooseRemotePort(String allowPort) throws ApiException {
        if (allowPort == null || allowPort.trim().isEmpty()) {
